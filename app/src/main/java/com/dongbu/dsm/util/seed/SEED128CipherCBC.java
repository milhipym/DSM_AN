...
public class SEED128CipherCBC
{
	  ...
	  private static int[] KC = { 
	    -1640531527, 1013904243, 2027808486, -239350324, -478700647, -957401293, -1914802585, 465362127, 
	    930724254, 1861448508, -572070280, -1144140559, 2006686179, -281594938, -563189875, -1126379749 };
	
	  private static boolean LITTLEENDIAN = false;
	  public static int[] m_RoundKey;
	  public byte[] m_Result;
	  public int[] m_ResultLen;
	
	  private static int GetB0(int A) {return 0xFF & A; } 
	  private static int GetB1(int A) { return 0xFF & A >>> 8; } 
	  private static int GetB2(int A) { return 0xFF & A >>> 16; } 
	  private static int GetB3(int A) { return 0xFF & A >>> 24; } 
	  private static void EndianChange(int[] dws) { dws[0] = (dws[0] >>> 24 | dws[0] << 24 | dws[0] << 8 & 0xFF0000 | dws[0] >>> 8 & 0xFF00); } 
	  private static int EndianChange(int dws) { return dws >>> 24 | dws << 24 | dws << 8 & 0xFF0000 | dws >>> 8 & 0xFF00; }
	
	
	  private static void SeedRound(int[] L0, int[] L1, int[] R0, int[] R1, int[] K)
	  {
		    long T00 = 0L; long T11 = 0L;
	  ...
	  public static void SeedEncrypt(byte[] pbData, int[] pdwRoundKey, byte[] outData)
	  {
		    ...
		    if (LITTLEENDIAN) { EndianChange(L0); EndianChange(L1); EndianChange(R0); EndianChange(R1);
		    }
	  ...
	  public static void SeedRoundKey(int[] pdwRoundKey, byte[] pbUserKey)
	  {
		    ...
		    if (LITTLEENDIAN) { A[0] = EndianChange(A[0]); B[0] = EndianChange(B[0]); C[0] = EndianChange(C[0]); D[0] = EndianChange(D[0]);
		    }
		    int T0 = A[0] + C[0] - KC[0];
		    int T1 = B[0] - D[0] + KC[0];
	  ...
	  public static void SEEDXor(byte[] out, byte[] in1, byte[] in2, int iCount)
	  {		
	  ...
	  public static byte[] stringToBytes(String str)
	  {
		   ...
		   {
			   int x = str.charAt(i);
			   if(LITTLEENDIAN) // 리틀 인디안
			   {
				   bytes[i*3]   = (byte)GetB0(x);
				   bytes[i*3+1] = (byte)GetB1(x);
				   bytes[i*3+2] = (byte)GetB2(x);
			   }
			   else
			   {
				   bytes[i*3]   = (byte)GetB2(x);
				   bytes[i*3+1] = (byte)GetB1(x);
				   bytes[i*3+2] = (byte)GetB0(x);
			   }			 
		   } 
		   return bytes;
	  }
	  ...
	  public static String bytesToString(byte [] bytes)
	  {
		   ...
		   for (int i = 0; i < bytes.length;) 
		   {
			    ...
			    if(LITTLEENDIAN) // 리틀 인디안
				{
			    	x = bytes[i++] & 0xFF  | (bytes[i++] & 0xFF) << 8  | (bytes[i++]& 0xFF) << 16 ;	
				}
			    else
			    {
			    	x = bytes[i++] & 0xFF << 16  | (bytes[i++] & 0xFF) << 8  | bytes[i++]& 0xFF;
			    }
			      
			    ret += new Character((char)x).toString();
			}
			return ret;
	   }
}
